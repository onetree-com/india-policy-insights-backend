...
{
    public class GetImprovementRanking : BaseFunction
    {
        private readonly IPdfReportsRepository _repository;

        public GetImprovementRanking(IPdfReportsRepository repository)
        {
            _repository = repository;
        }

        [FunctionName(nameof(GetImprovementRanking))]
            ...
            nameof(HttpMethods.Post), Route = null)] HttpRequest request)
        {
            try
            {
                ...
                GetImprovementRankingRequestDto s = JsonConvert.DeserializeObject<GetImprovementRankingRequestDto>(content);

                Log.Debug("Handling request for improvement ranking");
                var data = await _repository.GetImprovementRanking(s).ConfigureAwait(false);
