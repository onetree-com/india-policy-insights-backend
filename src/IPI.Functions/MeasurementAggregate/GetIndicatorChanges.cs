...
{
    public class GetIndicatorChanges : BaseFunction
    {
        private readonly IIndicatorChangeRepository _repository;

        public GetIndicatorChanges(IIndicatorChangeRepository repository)
        {
            _repository = repository;
        }

        [FunctionName(nameof(GetIndicatorChanges))]
            ...
            nameof(HttpMethods.Post), Route = null)] HttpRequest request)
        {
            try
            {
                ...
                var data = await _repository.GetIndicatorsChange(s).ConfigureAwait(false);
